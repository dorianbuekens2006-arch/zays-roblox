...
import type { ExecutorStatus, GameStatus, ScriptCategory, SiteContent } from "./types";
...
export function validateContent(value: unknown): SiteContent {
  ...
  return {
    ...
    links: {
      discord: url(links.discord),
      roblox: url(links.roblox),
      youtube: url(links.youtube),
      tiktok: url(links.tiktok),
      x: url(links.x),
      other: url(links.other),
    },
    settings: {
