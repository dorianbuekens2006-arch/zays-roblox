...
import type { ExecutorStatus, GameStatus, LinkCategory, ScriptCategory, SiteContent } from "./types";
...
export function validateContent(value: unknown): SiteContent {
  ...
  const links = array(root.links).slice(0, 100).map((entry) => {
    const link = object(entry);
    return {
      id: id(link.id),
      name: text(link.name, "Nouveau lien", 100),
      url: url(link.url),
      icon: text(link.icon, "link", 30),
      category: oneOf<LinkCategory>(link.category, ["Communauté", "Réseaux Sociaux", "Bot", "Autres"], "Autres"),
