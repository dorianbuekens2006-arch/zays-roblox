...
export default function PublicSite({ content }: { content: SiteContent }) {
  ...
  const activeLinks = content.links.filter((link) => link.active && link.url);
  const discordLink = activeLinks.find((link) => link.name.toLowerCase().includes("discord")) || null;
  const socialLinks = activeLinks.filter((link) => link !== discordLink);
  ...
  return (
    <div className="public-shell" style={style}>
      ...
      <main>
        ...
        <section id="discord" className="section community-section">
          <div className="community-card reveal">
            ...
            {discordLink ? (
              <a className="button button-discord" href={discordLink.url} target="_blank" rel="noreferrer" onClick={() => event("discord_clicks")}><Icon name="discord" />{content.communityButton}<Icon name="arrow" /></a>
      ...
      <footer className="footer">
        <div className="footer-main"><div><a className="brand footer-brand" href="#accueil"><span>{content.settings.logoText}</span><i /></a><p>{content.settings.description}</p>{socialLinks.length > 0 && <div className="footer-socials">{socialLinks.map((link) => <a href={link.url} target="_blank" rel="noreferrer" key={link.id} title={link.category}><Icon name={link.icon || "link"} />{link.name}</a>)}</div>}</div><nav><a href="#accueil">Accueil</a><a href="#jeux">Jeux</a><a href="#scripts">Scripts</a><a href="#discord">Discord</a></nav><nav><Link href="/conditions">Conditions d’utilisation</Link><Link href="/confidentialite">Politique de confidentialité</Link></nav></div>
