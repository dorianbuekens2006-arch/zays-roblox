...
export default function PublicSite({ content }: { content: SiteContent }) {
  ...
  const activeLinks = content.links.filter((link) => link.active && Boolean(link.url));
  const activeBots = content.bots.filter((bot) => bot.active);
  const discordLink = activeLinks.find((link) => link.category === "Discord")?.url || "";
  const socialLinks = activeLinks.filter((link) => link.category !== "Discord");
    ...
    <div className="public-shell" style={style}>
      <header className="navbar">
        ...
        <nav className={menuOpen ? "nav-links is-open" : "nav-links"} aria-label="Navigation principale">
          ...
          {activeLinks.length > 0 && <a href="#liens" onClick={() => setMenuOpen(false)}>Liens</a>}
          {activeBots.length > 0 && <a href="#bots" onClick={() => setMenuOpen(false)}>Bots</a>}
      ...
      <main>
        ...
        {activeLinks.length > 0 && (
          <section id="liens" className="section links-section">
            <div className="section-heading reveal">
              <div><p className="overline">DESTINATIONS</p><h2>Liens <em>Zays</em></h2></div>
              <p>Tous les liens officiels ajoutés et organisés directement depuis le panel admin.</p>
            </div>
            <div className="scripts-grid">
              {activeLinks.map((link, index) => (
                <article className="script-card reveal" key={link.id} style={{ transitionDelay: `${index * 70}ms` }}>
                  <div className="script-top"><div className="script-icon">{link.image.startsWith("/") || link.image.startsWith("http") ? <img src={link.image} alt="" /> : link.image}</div><Status value={link.status} /></div>
                  <div><span className="script-category">{link.category}</span><h3>{link.name}</h3><p>{link.description}</p></div>
                  <div className="script-details"><span>TYPE</span><b>{link.category}</b><span>MISE À JOUR</span><b>{new Date(link.updatedAt).toLocaleDateString("fr-FR")}</b></div>
                  <div className="card-actions"><a className="button button-primary button-small" href={link.url} target="_blank" rel="noreferrer"><Icon name="external" />Ouvrir</a></div>
                </article>
              ))}
            </div>
          </section>
        ...
        {activeBots.length > 0 && (
          <section id="bots" className="section bots-section">
            <div className="section-heading reveal">
              <div><p className="overline">AUTOMATISATION</p><h2>Bots <em>Zays</em></h2></div>
              <p>Les bots et outils automatisés que tu choisis de publier depuis le panel.</p>
            </div>
            <div className="scripts-grid">
              {activeBots.map((bot, index) => (
                <article className="script-card reveal" key={bot.id} style={{ transitionDelay: `${index * 70}ms` }}>
                  <div className="script-top"><div className="script-icon">{bot.image.startsWith("/") || bot.image.startsWith("http") ? <img src={bot.image} alt="" /> : bot.image}</div><Status value={bot.status} /></div>
                  <div><span className="script-category">{bot.platform}</span><h3>{bot.name}</h3><p>{bot.description}</p></div>
                  <div className="script-details"><span>PLATEFORME</span><b>{bot.platform}</b><span>STATUT</span><b>{bot.status}</b></div>
                  <div className="card-actions">{bot.url ? <a className="button button-primary button-small" href={bot.url} target="_blank" rel="noreferrer"><Icon name="external" />{bot.buttonText}</a> : <button className="button button-primary button-small" disabled>{bot.buttonText}</button>}</div>
                </article>
              ))}
            </div>
          </section>
        ...
        <section id="discord" className="section community-section">
          <div className="community-card reveal">
            ...
            {discordLink ? (
              <a className="button button-discord" href={discordLink} target="_blank" rel="noreferrer" onClick={() => event("discord_clicks")}><Icon name="discord" />{content.communityButton}<Icon name="arrow" /></a>
            ) : <button className="button button-discord" disabled title="Ajoute et active un lien de catégorie Discord dans le panel admin"><Icon name="discord" />{content.communityButton}</button>}
          </div>
        </section>
      </main>

      <footer className="footer">
        <div className="footer-main"><div><a className="brand footer-brand" href="#accueil"><span>{content.settings.logoText}</span><i /></a><p>{content.settings.description}</p>{socialLinks.length > 0 && <div className="footer-socials">{socialLinks.map((link) => <a href={link.url} target="_blank" rel="noreferrer" key={link.id}>{link.name}</a>)}</div>}</div><nav><a href="#accueil">Accueil</a><a href="#jeux">Jeux</a><a href="#scripts">Scripts</a>{activeLinks.length > 0 && <a href="#liens">Liens</a>}{activeBots.length > 0 && <a href="#bots">Bots</a>}<a href="#discord">Discord</a></nav><nav><Link href="/conditions">Conditions d’utilisation</Link><Link href="/confidentialite">Politique de confidentialité</Link></nav></div>
